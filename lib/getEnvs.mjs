 ...
 * @property {string|undefined} AFTER_FIRST_SCRIPT
 */
...
const askQuestions = async (
  ...
  let startScriptQuestion;

  // for some reason, `validate` doesn't work for autocomplete...
  while (
    !startScriptQuestion ||
    !scriptChoices.some(({ title }) => title === startScriptQuestion?.script)
  )
    startScriptQuestion = await prompts(
      {
        type: 'autocomplete',
        name: 'script',
  ...
  const START_SCRIPT = startScriptQuestion.script;

  const NONE_OPTION = Symbol('option for "none"');
  /** @type {prompts.Answers<"script">|undefined} */
  let afterFirstScriptQuestion;
  // for some reason, `validate` doesn't work for autocomplete...
  while (
    !afterFirstScriptQuestion ||
    (afterFirstScriptQuestion.script !== NONE_OPTION &&
      !scriptChoices.some(
        ({ title }) => title === afterFirstScriptQuestion?.script,
      ))
  ) {
    afterFirstScriptQuestion = await prompts(
      {
        type: 'autocomplete',
        name: 'script',
        message: 'After first init script',
        initial: process.env.AFTER_FIRST_SCRIPT,
        choices: [
          { title: kleur.italic('skip'), value: NONE_OPTION },
          ...scriptChoices,
        ],
      },
      { onCancel },
  ...
  const AFTER_FIRST_SCRIPT =
    afterFirstScriptQuestion.script === NONE_OPTION
      ? undefined
      : afterFirstScriptQuestion.script;
  ...
  if (saveAnswers) {
    ...
    await fs.writeFile(
      ...
      [
        ...
        AFTER_FIRST_SCRIPT
          ? `AFTER_FIRST_SCRIPT="${AFTER_FIRST_SCRIPT}"`
          : undefined,
      ]
        .filter(Boolean)
        .join(os.EOL) + os.EOL,
